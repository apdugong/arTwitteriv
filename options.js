...
function renderFields() {
  ...
  fields.forEach((field, index) => {
    ...
    row.innerHTML = `<label>表示名<input class="field-label" value=""></label><label>arXiv検索式<textarea class="field-query" rows="3"></textarea></label><button type="button" class="secondary remove-field">削除</button>`;
...
function renderDefaultOptions() {
  ...
  fields.forEach(field => { const option = document.createElement('option'); option.value = field.id; option.textContent = field.label || '名称未設定'; defaultField.appendChild(option); });
...
(async () => {
  ...
  ['authorFilter','batchSize','randomStartDate','randomEndDate','randomMinCitations','randomMaxCitations','classicsStartDate','classicsEndDate','classicsMinCitations','classicsMaxCitations'].forEach(id => setValue(id, settings[id]));
})();
...
form.addEventListener('submit', async event => {
  ...
  if (!cleaned.length) { savedMessage.textContent = '分野を1つ以上設定してください'; return; }
  ...
  savedMessage.textContent = '保存しました'; setTimeout(() => { savedMessage.textContent = ''; }, 1800);
});
